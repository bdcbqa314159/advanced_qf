...
struct result_data
{
    ...
    result_data(double alpha, double dtau, double k, vec x, vec S, vec t, vec tau, matrix<double> u, matrix<double> v)
        : alpha(alpha), dtau(dtau), k(k), x(x), S(S), t(t), tau(tau), u(u), v(v)
    {
    }
